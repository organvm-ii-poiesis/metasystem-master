...
const {weighted:w, types:t, typescriptVersion} = require('./isolated-loader.cjs');
...
const output={schemaVersion:'1.0',sourceRevision:'323c12f6c17753b0b408a3f0e34265939395d864',generatedAt:new Date().toISOString(),
 executionClass:'dependency_isolated_original_source',runtime:{node:process.version,typescript:typescriptVersion,platform:process.platform,architecture:process.arch},
 assistantProduced:true,canonicalSuiteExecuted:false,githubActionsExecuted:false,minority,temporal,expiry,overrideModes,invalid,evenMedian,majorityTies};
...
fs.writeFileSync(`${__dirname}/results/experiments.json`,JSON.stringify(output,replacer,2)+'\n');
console.log(JSON.stringify({minorityCases:minority.length,defaultSuppressed:minority.filter(x=>x.threshold===2.5&&x.consensusGamma===0.2&&x.retainedMinority===0).map(x=>x.minorityCount),temporal,expiry,invalid,evenMedian,majorityTies},replacer,2));
